...
'''
   ...
   v1:
   updata metis clustering method
...
#         assert type(num_clusters) == int
        ...
        self.data = data
        self.adjacency_dict = data.adjacency_dict
    ...
    def run(self):
        ...
        for key,value in self.adjacency_dict.items():
            for i in range(len(value)):
                cust_id.append(key)
                opp_id.append(i)
        self.edge_df = pd.DataFrame({'cust_id':cust_id, 'opp_id':opp_id})
        features_df = pd.DataFrame(data = self.data.x, columns = ['f' + str(i) for i in range(1433)])
        info_df = pd.DataFrame({'cust_id':range(self.data.x.shape[0]),'is_driver':self.data.test_mask,'is_reported':self.data.y})
        self.node_df = pd.concat([info_df,features_df],axis = 1)
    ...
    def metis_clustering(self):
        ...
        adjacency_list = []
        for node in range(len(self.adjacency_dict.keys())+1):
            adjacency_list.append(self.adjacency_dict[node])
        _, membership= pymetis.part_graph(self.num_clusters, adjacency_list)
        cluster_membership = {}
        for node, member in zip(list(range(len(self.adjacency_dict.keys())+1)), membership):
...
class run_model():
    ...
    def __init__(self,
        ...
        pre_model = preprocessing(self.data, self.num_clusters, self.cluster_method)
...
#         print('train_nodes', train_nodes.shape)
    ...
    def train(self):
        ...
        print('best accuracy is: ',accuracy_max)
        if self.save_path:
    ...
    def test(self):
        ...
        return sum(self.predictions == self.targets)/len(self.targets)